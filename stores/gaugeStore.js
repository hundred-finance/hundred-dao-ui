...
class Store {
    ...
    dispatcher.register((payload) => {
      switch (payload.type) {
        ...
        case APPLY_BOOST:
          ...
          break;
        default: {
          /* continue regardless of eslint error */
        }
      }
    });
  }
  ...
  async configure(payload) {
    ...
    async.map(
      ...
      (project, callback) => {
        this.getProjectData(project, callback);
      },
      (err, project) => {
        if (err) {
  ...
  async getProjectData(project, callback) {
    const web3 = await stores.accountStore.getWeb3Provider();
    ...
    const hndPrice = await this.getHndPrice();
    ...
    const lpTokens = gaugesLPTokens.map((gauge, index) => {
      ...
      return {
        ...
        decimals: data[2],
        balance: data[3],
        underlyingDecimals: data[4],
        underlyingSymbol: data[5],
      };
    });

    const projectGauges = [];
    for (let i = 0; i < gauges.length; i++) {
      const lpPrice = lpTokenUnderlyingInfo[i].price / 10 ** (36 - lpTokens[i].underlyingDecimals);
      const convRate = lpTokenUnderlyingInfo[i].exchangeRate / 1e18;
    ...
    const projectVeTokenMetadata = {
      ...
      decimals: Number(veTokenMetadata.decimals),
  ...
  async getTokenBalances(payload) {
    ...
    for (let i = 0; i < project.gauges.length; i++) {
      ...
      totalPercentUsed += gaugeVotePercent;
      ...
      const providedLiquidity = project.gauges[i].balance * project.gauges[i].lpToken.price;
      const totalProvidedLiquidity = project.gauges[i].totalStakeBalance * project.gauges[i].lpToken.price;

      const totalRewards = (currentRewardRate * 365 * 24 * 3600 * project.hndPrice) / 1e18;
      const gaugeRewards = (totalRewards * project.gauges[i].currentEpochRelativeWeight) / 100;
      const rewards = gaugeRewards * project.gauges[i].liquidityShare;

      const nextEpochTotalRewards = (nextEpochRewardRate * 365 * 24 * 3600 * project.hndPrice) / 1e18;
      const nextEpochGaugeRewards = (nextEpochTotalRewards * project.gauges[i].nextEpochRelativeWeight) / 100;
      const nextEpochRewards = nextEpochGaugeRewards * project.gauges[i].liquidityShare;
  ...
  async approveLock(payload) {
    ...
    this.callApproveLock(web3, project, account, amount, (err, approveResult) => {
      if (err) {
  ...
  async callApproveLock(web3, project, account, amount, callback) {
    ...
    await this.asyncCallContractWait(
      web3,
      tokenContract,
      'approve',
  ...
  async lock(payload) {
    ...
    this.callLock(web3, project, account, amount, selectedDate, (err, lockResult) => {
      if (err) {
        return this.emitter.emit(ERROR, err);
      }
  ...
  async unlock(payload) {
    ...
    this.callUnlock(web3, project, account, (err, lockResult) => {
      if (err) {
        return this.emitter.emit(ERROR, err);
      }
  ...
  async callLock(web3, project, account, amount, selectedDate, callback) {
  ...
  async callUnlock(web3, project, account, callback) {
    const escrowContract = new web3.eth.Contract(VOTING_ESCROW_ABI, project.votingEscrow);

    await this.asyncCallContractWait(web3, escrowContract, 'withdraw', [], account, null, GET_TOKEN_BALANCES, { id: project.id }, callback);
  }
  ...
  async vote(payload) {
    ...
    this.calVoteForGaugeWeights(web3, project, account, amount, gaugeAddress, (err, voteResult) => {
      if (err) {
        return this.emitter.emit(ERROR, err);
      }
  ...
  async calVoteForGaugeWeights(web3, project, account, amount, gaugeAddress, callback) {
  ...
  async increaseLockAmount(payload) {
    ...
    this.callIncreaseAmount(web3, project, account, amount, (err, lockResult) => {
      if (err) {
        return this.emitter.emit(ERROR, err);
      }
  ...
  async callIncreaseAmount(web3, project, account, amount, callback) {
    const escrowContract = new web3.eth.Contract(VOTING_ESCROW_ABI, project.votingEscrow);
    ...
    await this.asyncCallContractWait(web3, escrowContract, 'increase_amount', [amountToSend], account, null, GET_TOKEN_BALANCES, { id: project.id }, callback);
  }
  ...
  async increaseLockDuration(payload) {
    ...
    this.callIncreaseUnlockTime(web3, project, account, selectedDate, (err, lockResult) => {
      if (err) {
        return this.emitter.emit(ERROR, err);
      }
  ...
  async applyBoost(payload) {
    ...
    const { gaugeAddress, project } = payload.content;
  ...
  async callIncreaseUnlockTime(web3, project, account, selectedDate, callback) {
    const escrowContract = new web3.eth.Contract(VOTING_ESCROW_ABI, project.votingEscrow);
  ...
  async asyncCallContractWait(web3, contract, method, params, account, gasPrice, dispatchEvent, dispatchEventPayload, callback) {
    const sendPayload = {
      from: account.address,
    };

    await this.callContractWait(web3, contract, method, params, account, sendPayload, dispatchEvent, dispatchEventPayload, callback);
  }

  async callContractWait(web3, contract, method, params, account, sendPayload, dispatchEvent, dispatchEventPayload, callback) {
    const context = this;

    const chainId = await web3?.eth?.getChainId();
    const chain = NETWORKS_CONFIG.find((chain) => Number(chain.chainId) === chainId);

    contract.methods[method](...params)
      .send(sendPayload)
      .on('transactionHash', (hash) => {
        context.emitter.emit(TX_SUBMITTED, { hash, baseUrl: chain.blockExplorerUrls[0] });
      })
      .on('receipt', (receipt) => {
        // console.log(receipt);
...
function userRemainingStake(balance, totalBalance, veTokenBalance, totalVeTokenSupply) {
  const currentStake = balance;

  const maxStake = (totalBalance * veTokenBalance) / totalVeTokenSupply;
