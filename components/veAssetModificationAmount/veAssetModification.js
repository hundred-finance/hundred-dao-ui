...
import {
  ERROR, INCREASE_LOCK_AMOUNT, INCREASE_LOCK_AMOUNT_RETURNED, APPROVE_LOCK, APPROVE_LOCK_RETURNED,
  ...
  return (
    <Paper elevation={1} className={classes.projectCardContainer}>
      <Typography variant="h3" className={classes.sectionHeader}>
        Lock additional
        {project && project.tokenMetadata ? project.tokenMetadata.symbol : 'asset'}
      </Typography>
        ...
        <div className={classes.calculationResults}>
          <div className={classes.calculationResult}>
            <Typography variant="h3">
              You will receive:
              {projectedVeHndBalance(project, amount).toFixed(2)}
              {' '}
              veHND
            </Typography>
          </div>
...
function projectedVeHndBalance(project, amount) {
  const lockEnd = project.veTokenMetadata.userLockEnd;
