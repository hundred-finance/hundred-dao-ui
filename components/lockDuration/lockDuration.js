...
function LockDurationChart({ project }) {
  ...
  const totalSupply = project?.veTokenMetadata.totalLocalSupply;
  const totalLocked = project?.veTokenMetadata.supply;
    ...
    <Paper elevation={1} className={classes.overviewContainer}>
      <ChartContainer>
        ...
        <div>
          ...
          {' '}
          <span style={{ color: accentColor }}>{years}</span>
          {' '}
          years (based on
          {' '}
          {toFixedIfNecessary(totalSupply, 2)}
          {' '}
          {project.veTokenMetadata.symbol}
          {' '}
          and
          {toFixedIfNecessary(totalLocked, 2)}
          {' '}
