...
export default function BoostCalculator({ project }) {
  ...
  const userLiquidityShare = (gauge, balance, veTokenBalance, totalVeTokenSupply) => {
    ...
    let totalWorkingSupply = (gauge.workingSupply / 10 ** gauge.lpToken.underlyingDecimals) * gauge.lpToken.conversionRate;
    ...
    <Paper elevation={1} className={classes.projectCardContainer}>
      ...
      <div>
        ...
        <div className={classes.boost}>
          ...
          <div>
            <Typography variant="h3">Estimated boost: {formatCurrency(calculatedBoost(project, gauge, stakeAmount, lockAmount, selectedDate))}</Typography>
